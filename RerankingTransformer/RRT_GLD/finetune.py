...
@ex.automain
def main(epochs, cpu, cudnn_flag, visdom_port, visdom_freq, temp_dir, seed, no_bias_decay, max_norm, resume, save_name):
    ...
    #     print("Distributing model using Data Parallel on ", torch.cuda.device_count(), "GPUs!")
    ...
    eval_function = partial(evaluate, model=model, 
        ...
        recall=recall_ks, query_loader=loaders.query, gallery_loader=loaders.gallery, train=True, limit=5)
    ...
    # saving
    save_name = osp.join(temp_dir, '{}_{}_{}.pt'.format(ex.current_run.config['model']['name'],
                                                         ex.current_run.config['dataset']['name'], save_name))
    ...
    for epoch in range(epochs):
        ...
        ex.log_scalar('val.map', result, step=epoch + 1)

        if result >= best_val[1]:
        ...
        # if epoch == epochs//2:
        #     torch.save({'state': state_dict_to_cpu(best_val[2]), 'optim': optimizer.state_dict()}, "halfway_" + save_name)
